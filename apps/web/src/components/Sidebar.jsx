...
const SidebarIcon = ({ icon, isDark }) => {
  const iconClass = `text-4xl ${isDark ? 'text-white' : ''}`;
...
export default function Sidebar() {
  ...
  if (isLoading) {
    return (
      <aside className="sidebar w-2/12 h-screen relative shadow-xl flex items-center justify-center">
  ...
  return (
    <aside className="sidebar w-2/12 h-screen relative shadow-xl overflow-y-auto">
      <nav>
        <ul className="flex flex-col gap-4 p-4 items-start text-xl mt-8 ml-5">
          ...
          {sidebarItems.map((item) => (
            <li key={item.path} className="flex items-center gap-4">
              <Link 
                to={item.path} 
                className={`flex items-center gap-4 hover:opacity-80 transition-opacity ${isDark ? 'text-white' : ''} ${largeText ? 'text-2xl' : ''} ${dyslexiaFont ? 'font-dyslexia' : ''}`}
              >
                <SidebarIcon icon={item.icon} isDark={isDark} />
                {item.label}
              </Link>
            </li>
          ))}

          {/* Accesibilidad (siempre visible para todos los roles) */}
          <li className="flex flex-col items-start w-full">
            <button
              onClick={() => setOpenAccesibilidad(!openAccesibilidad)}
              className={`flex items-center justify-between w-full hover:opacity-80 transition-opacity ${isDark ? 'text-white' : ''} ${largeText ? 'text-2xl' : ''} ${dyslexiaFont ? 'font-dyslexia' : ''}`}
            >
              <div className="flex items-center gap-4">
                <svg className="text-4xl" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 512 512">
              ...
              <svg className={`text-2xl transform transition-transform duration-200 ${openAccesibilidad ? 'rotate-180' : 'rotate-0'} ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
            ...
            {openAccesibilidad && (
              <ul className={`ml-18 text-base mt-1 space-y-1 w-10/12 ${isDark ? 'text-white' : ''} ${largeText ? 'text-xl' : ''} ${dyslexiaFont ? 'font-dyslexia' : ''}`}>
                ...
                <li onClick={toggleTheme} className={`flex items-center gap-1 mt-2 cursor-pointer hover:opacity-80 transition-opacity ${isDark ? 'text-white' : ''}`}>
                  <svg className={`text-2xl ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                    <path fill="currentColor" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.353 3C5.849 4.408 3 7.463 3 11.47A9.53 9.53 0 0 0 12.53 21c4.007 0 7.062-2.849 8.47-6.353C8.17 17.065 8.14 8.14 9.353 3"/>
                  </svg>
                  {isDark ? 'Modo Claro' : 'Modo Oscuro'}
                </li>
                <li onClick={toggleLargeText} className={`flex items-center gap-1 mt-2 cursor-pointer hover:opacity-80 transition-opacity ${isDark ? 'text-white' : ''}`}>
                  <svg className={`text-2xl ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 512 512">
                    <path fill="currentColor" d="M32 256h192v64h-64v192H96V320H32zm448-128H354.125v384h-68.25V128H160V64h320z"/>
                  </svg>
                  {largeText ? 'Texto normal' : 'Texto grande'}
                </li>
                <li onClick={toggleHighContrast} className={`flex items-center gap-1 mt-2 cursor-pointer hover:opacity-80 transition-opacity ${isDark ? 'text-white' : ''}`}>
                  <svg className={`text-2xl ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M5 21q-.825 0-1.412-.587T3 19V5q0-.825.588-1.412T5 3h14q.825 0 1.413.588T21 5v14q0 .825-.587 1.413T19 21zm0-2h14V5zm12.025-3h-5v-1.5h5zM5.5 9h2v2H9V9h2V7.5H9v-2H7.5v2h-2z"/>
                  </svg>
                  {highContrast ? 'Contraste normal' : 'Alto contraste'}
                </li>
                <li onClick={toggleDyslexiaFont} className={`flex items-center gap-1 mt-2 cursor-pointer hover:opacity-80 transition-opacity ${isDark ? 'text-white' : ''}`}>
                  <svg className={`text-2xl ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 16 16">
          ...
          <li className={`flex items-center gap-4 ${isDark ? 'text-white' : ''} ${largeText ? 'text-2xl' : ''} ${dyslexiaFont ? 'font-dyslexia' : ''}`}>
            <button className="flex items-center gap-4 hover:opacity-80 transition-opacity">
              <svg className={`text-4xl ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 48 48">
                <defs>
