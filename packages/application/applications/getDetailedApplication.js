...
export async function getDetailedApplication(applicationUuid) {
  ...
  try {
    ...
    const { bannerUrl, attachments } = _normalizeFiles(fileLinks);
 ...
 */
function _normalizeFiles(fileLinks) {
  let bannerUrl = null;
  const attachments = [];

  for (const link of fileLinks) {
    const url = buildFileUrl(link);

    if (link.purpose === "BANNER" && !bannerUrl) {
      // Only take the first banner
      bannerUrl = url;
    } else if (link.purpose === "ATTACHMENT") {
      attachments.push({
        url,
